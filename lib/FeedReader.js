...
function buildConfig (newConfig) {
    ...
    _.assign(config, _.omit(newConfig, 'winston'));
...
FeedReader.prototype.updateConfig = function (newConfig, callback) {
    ...
    eventCreator.emitFeedUpdateStatus({ 
        debug: 'GTFS-Realtime FeedReader config update started.', 
        timestamp: (Date.now() + (process.hrtime()[1]%1000000)/1000000) ,
    });
    ...
    try {
         ...
         */
            ...
            try {
                ...
                eventCreator.emitError({ error: err, timestamp: (Date.now() + (process.hrtime()[1]%1000000)/1000000) });
        ...
        if (this.control.retryReadIntervalObj) { // If running, restart.
            restart.call(this);
        } 
...
function start () {
    ...
    if ( ! this.control.feedReaderIntervalObj ) { // Only have one intervaledFeedReader running at a time.
        ...
        this.control.feedReaderIntervalObj = this.control.feedReaderIntervalObj || 
            setInterval(intervaledFeedReader.bind(this), (this.config.readInterval * 1000));
...
function intervaledRetry () {
    ...
    if (++this.control.retryCounter > this.config.MaxNumRetries) {
        clearInterval(this.control.retryReadIntervalObj);
    } else if (this.control.retryReadIntervalObj) {
        readFeed.call(this);
    }
}
...
function parse (res) {
    ...
    res.on("end", function() {
        try {
            ...
            if ( (err.message !== messageSequenceErrorMessage) || 
                 (that.control.retryCounter === that.config.MaxNumRetries)) {

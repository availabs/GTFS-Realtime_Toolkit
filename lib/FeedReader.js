...
function FeedReader (config) {
    ...
    this.control.lastReadTimestamp     = null;
...
function buildConfig (newConfig) {
    ...
    config.retryInterval = (newConfig.retryInterval && !isNaN(newConfig.retryInterval)) || RETRY_DEFAULTS.retryInterval;
...
function resetResusitatorTimeout () {
    ...
    this.control.resuscitatorTimeoutIntervalObject = setTimeout(resuscitator.bind(this), 3000 * this.config.readInterval);
}
...
FeedReader.prototype.getLastReadTimestamp = function () {
    /*jshint validthis:true*/
    this.logger && this.logger.silly('getLastReadTimestamp called'); /* jshint ignore: line */
    return this.control.lastReadTimestamp;
};
...
FeedReader.prototype.registerListener = function (listener) {
    ...
    this.logger && this.logger.silly('listener added.'); /* jshint ignore: line */
...
FeedReader.prototype.removeListener = function (listener) {
    ...
    if (this.listeners.length === 0) { // If a tree falls in a forest...
        this.logger && this.logger.debug('in removeListener: all listeners removed.'); /* jshint ignore: line */
        stop.call(this);
    }
};
...
function start () {
    ...
    if ( ! this.control.feedReaderIntervalObj ) { // Only have one intervaledFeedReader running at a time.
        ...
        this.logger && this.logger.silly('in start: interval started'); /* jshint ignore: line */
    }
...
function restart () {
    ...
    setTimeout(function () {
        this.logger && this.logger.debug('inside restart timeout.'); /* jshint ignore: line */
        stop.call(that);
        start.call(that);
    }, this.config.readInterval * 1000); 
}
...
function intervaledFeedReader () {
    ...
    this.logger && this.logger.silly('intervaledFeedReader called.'); /* jshint ignore: line */
    clearInterval(this.control.retryReadIntervalObj);
    ...
    this.logger && this.logger.silly('after clearing retryReadIntervalObj.'); /* jshint ignore: line */
...
function retry () {
    /*jshint validthis:true */
    this.logger && this.logger.silly('retry called.'); /* jshint ignore: line */
    ...
    if ( ! this.control.retryReadIntervalObj ) {
        ...
        this.logger && this.logger.silly('called setIntervalForRetry.'); /* jshint ignore: line */
...
function intervaledRetry () {
    /*jshint validthis:true */
    this.logger && this.logger.silly('intervaledRetry called.'); /* jshint ignore: line */
...
function readFeed () {
    ...
    this.control.lastReadTimestamp = Math.floor(Date.now() / 1000);

    resetResusitatorTimeout.call(this);

    http.get(this.config.feedURL, parse.bind(this))
        .on("error", function (e) { 
            this.logger && this.logger.debug('http get error.', { e : e }); /* jshint ignore: line */
            retry.call(that);
        });
}
...
function parse (res) {
    ...
    res.on("data", function(chunk) {
        //that.logger && that.logger.silly('new data chunk.');/* jshint ignore: line */
        data.push(chunk);
    });

    res.on("end", function() {
        try {
            that.logger && that.logger.silly('data end.'); /* jshint ignore: line */
            data = Buffer.concat(data);
            
            var msg = that.feedMessageDecoder(data),
                timestamp = _.get(msg, ['header', 'timestamp'], null); 

            that.logger && that.logger.silly('data parsed.'); /* jshint ignore: line */

            if (timestamp <= that.control.previousTimestamp) {
                that.logger && that.logger.silly('timestamp is not newer.'); /* jshint ignore: line */
                ...
                that.logger && that.logger.silly('updated timestamp.'); /* jshint ignore: line */
            }

            clearInterval(that.control.retryReadIntervalObj);
            that.logger && that.logger.silly('cleared retryReadIntervalObj.'); /* jshint ignore: line */
            
            _.forEach(that.listeners, function (listener, i) { 
                try {
                    that.logger && that.logger.silly('calling listener #' + i); /* jshint ignore: line */
                    listener(msg); 
                    that.logger && that.logger.silly('listener #' + i + ' returned.'); /* jshint ignore: line */
                } catch (e) {
                    that.logger && that.logger.error('Uncaught error in a feed listener', { e: e }); /* jshint ignore: line */
                }
            });

            if (that.control.restartReader) {
                that.logger && that.logger.silly('restarting the feed reader.'); /* jshint ignore: line */
                restart.call(that);
            }

        } catch (e) {
            console.log("ERROR: retry#", that.control.retryCounter, ": parsing GTFS-Realtime:", e, '\n');

            that.logger && that.logger.error('error in parsing.', { e : e , retry: that.control.retryCounter } ); /* jshint ignore: line */

            if (that.control.restartReader) {
                that.logger && that.logger.silly('calling restart.'); /* jshint ignore: line */
                restart.call(that);
            } else {
                that.logger && that.logger.silly('calling retry.'); /* jshint ignore: line */
                retry.call(that);
            }
        } 
    }); 
    ...
    res.on("error", function(e) {
        that.logger && that.logger.error("ERROR: parse res.error. Assumed we wouldn't reach that. Calling retry.", 

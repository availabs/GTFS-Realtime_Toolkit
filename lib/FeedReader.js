...
var ProtoBuf = require('protobufjs'),
    ...
    eventsCreator = require('./events/EventsCreator') ,
...
function FeedReader (config) {
    ...
    if ( ! (config && config.feedURL && config.readInterval && config.protofilePath) ) {
        ...
        eventsCreator.emitFeedUpdateStatus({ 
            error: 'Invalid configuration for the GTFS-Realtime FeedReader.', 
            debug: configError.stack ,
            timestamp: Date.now() ,
        });

        eventsCreator.emitError({ error: configError, timestamp: Date.now() });
    ...
    try {
        ...
        eventsCreator.emitError({ error: err, timestamp: Date.now() });
...
FeedReader.prototype.updateConfig = function (newConfig, callback) {
    ...
    eventsCreator.emitFeedUpdateStatus({ debug: 'GTFS-Realtime FeedReader config update started.', timestamp: Date.now() });
    ...
    try {
        ...
        eventsCreator.emitFeedUpdateStatus({ info: 'GTFS-Realtime FeedReader config update complete.', timestamp: Date.now() });

...
function resuscitator () {
    ...
    eventsCreator.emitError({ warning: 'WARN: Must resuscitate the Feed reader.' });
...
FeedReader.prototype.updateConfig = function (newConfig, callback) {
    try {
        ...
        eventsCreator.emitError({ error: e.stack });
        callback(e);
    }
};
...
function parse (res) {
    ...
    res.on("end", function() {
        try {
            ...
            eventsCreator.emitError({ error: e.stack, retryNumber: that.control.retryCounter });
            retry.call(that);
        } 
    }); 

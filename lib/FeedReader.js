...
function buildConfig (newConfig) {
    ...
    config.feedURL       = newConfig.feedURL;
    config.readInterval  = newConfig.readInterval;
...
function resuscitator () {
    ...
    } else if (this.control.resuscitatorCounter === 50){
       restart.call(this);
    }
}
...
function readFeed () {
    ...
    http.get(this.config.feedURL, parse.bind(this))
        .on("error", function (e) { 
            console.log("ERROR: readFeed get.error.");
            console.log(e);
            retry.call(that);
        });
}
...
function parse (res) {
    ...
    res.on("error", function(e) {
        console.log("ERROR: parse res.error. Assumed we wouldn't reach this.");
        console.log(e);
        retry.call(that);
    });
}

...
function parse (res) {
    ...
    res.on("end", function() {
        try {
            ...
            console.log("ERROR parsing GTFS-Realtime:", e);

            if (badData) {
                that.logger && that.logger.error('error in parsing.', { e : e , data : data.toString() } ); /* jshint ignore: line */
            } else {
                that.logger && that.logger.error('error in parsing.', { e : e } ); /* jshint ignore: line */
            }

...
function indexTripUpdate (theIndices, tripUpdateMessage, GTFS) {
    ...
    tripLevelDelay = tripUpdateMessage.delay ;

    stopTimeUpdates = tripUpdateMessage.stop_time_update ;
    ...
    handleStopTimeUpdatesForTrip(GTFS, theIndices, stopTimeUpdates, tripID, tripLevelDelay);
}
...
//
function handleStopTimeUpdatesForTrip (GTFS, theIndices, stopTimeUpdates, tripKey, tripLevelDelay) {
    var update ,
    ...
    for ( i = 0; i < stopTimeUpdates.length; ++i ) {
        ...
        for ( j = 0; j < timeTypes.length; ++j ) {
            ...
            if ((!isNaN(delay)) && (scheduledTime !== null)) {
                ...
                // Need to use the date.
                scheduledTimestamp = timeUtils.getTimestamp(scheduledTime, "HH:mm:ss", 'X');

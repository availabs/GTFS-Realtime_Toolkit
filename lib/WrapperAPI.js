...
var theAPI = {
     ...
     *  @returns {number}
     */
    getTimestampForFeedMessage : function () {
        var timestamp = _.get(this, ['GTFSrt_JSON', 'header', 'timestamp'], null);

        return parseInt((timestamp && (timestamp.low || timestamp.high)) || timestamp) || null ;
    },


    getTimestampForTrip : function (tripKey) {
        var tripUpdateMessage = this.getTripUpdateForTrip(tripKey) ,
            timestamp = _.get(tripUpdateMessage, ['timestamp'], null) ;
    ...
    // Simple Identity function here. For easy overriding by extension modules.
    getGTFSTripKeyForRealtimeTripKey : function (tripKey) {
        return tripKey;
    },
    ...
    getStartDateForTrip : function (trip_id) {
        return _.get(this.getTripUpdateForTrip(trip_id), ['trip', 'start_date'], null);
    },
    ...
    getStopTimeUpdatesForTrip : function (trip_id) {
        var stopTimeUpdates = _.get(this, ['tripsIndex', trip_id, 'TripUpdate', 'stop_time_update'], null);

        return (Array.isArray(stopTimeUpdates) && stopTimeUpdates.length) ? stopTimeUpdates : null;
    },
    ...
    /**
     *  Each tripsIndex node's `stops` member maps a stop_id to the index of that stop's 
     *      StopTimeUpdate in the stop_time_update array.
     *
    ...
    getFirstStopTimeUpdateForTrip : function (trip_id) {
        return _.first(this.getStopTimeUpdatesForTrip(trip_id)) || null;
    },
    ...
    getOnwardStopIDsForTrip : function (trip_id) {
        return _.map(this.getStopTimeUpdatesForTrip(trip_id), 'stop_id');
    },
    ...
    /* The assumption that the last stop in the StopTimeUpdates represents the destination 
     * is not sound outside of MTA Subway. 
    /**
     ...
     *  @return {object}
     */
    getDestinationStopIdForTrip : function (trip_id) {
    ...
    getCurrentStopSequenceForTrip : function (trip_id) {
        /* jslint validthis:true */
        var firstStopTimeUpdate = this.getFirstStopTimeUpdateForTrip(trip_id),
            stopSequence = parseInt(firstStopTimeUpdate && firstStopTimeUpdate.stop_sequence) ,
            vehiclePosition ;

        if (isNaN(stopSequence)) {
            vehiclePosition = this.getVehiclePositionUpdateForTrip(trip_id); 
            stopSequence = parseInt(vehiclePosition && vehiclePosition.current_stop_sequence);
        }

        return (!isNaN(stopSequence)) ? stopSequence : null;
    }, 

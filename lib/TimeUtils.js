...
function getTimestamp (time, parseFormat, outputFormat) {
    ...
    if (agency_timezone) {
        fT = (outputFormat) ? t.tz(agency_timezone).format(outputFormat) : t.tz(agency_timezone).format();
    } else {
        fT = (outputFormat) ? t.utc().format(outputFormat) : t.utc().format('YYYY-MM-DDTHH:mm:ssZ');
    }
...
function getTimeStampForHundredthsOfMinutePastMidnight (scheduleDate, hundredthsOfMinutePastMidnight) {
    ...
    // NOTE: getTimestamp will return null if the parsed time is invalid.
    //       This logic is currently relied upon in a hacky way by getOriginTimeForTrip. 
